...
///		public static var renderer:Renderer;
		...
		public static List<double> xpTable;	/*Number*/
...
///		public static var runeNameDeck:Array;
		public static int sewersFirstLevel;
...
///		public var yendor:Item;
		...
		public const int TOTAL_AREAS = 2;
		
		public const double CURSED_CHANCE = 0.1d;
		public const double BLESSED_CHANCE = 0.9d;
		public const double XP_TABLE_SEED = 10d;
		public const double XP_RATE = 2d;
		public const double MONSTER_XP_BY_LEVEL_RATE = 1.2d;
		public const int UNDERWORLD_PORTAL_LEVEL = 11;
...
///			var i:int, j:int;
		...
		public int equipmentQuantityPerLevel(int level) {
			if(level <= 0) return 0;
			if(level > TOTAL_LEVELS) level = TOTAL_LEVELS;
			double n = (level + Map.random.range(3)) * 0.5d;
			//return n == (n >> 0) ? n : (n >> 0) + 1; // inline Math.ceil()
			return (int)Math.Ceiling( n );
		}
		
		public int runeQuantityPerLevel(int level) {
			if(level <= 0) return 0;
			if(level > TOTAL_LEVELS) level = TOTAL_LEVELS;
			double n = (level + Map.random.range(2)) * 0.5d;
			//return n == (n >> 0) ? n : (n >> 0) + 1; // inline Math.ceil()
			return (int)Math.Ceiling( n );
		}
		
		public int monsterQuantityPerLevel(int level) {
			if(level <= 0) return 0;
			if(level > TOTAL_LEVELS) level = TOTAL_LEVELS;
			double n = level * (2 + Map.random.range(2)) * 0.6d;
			if(n > 24) n = 24;
			// I have no idea what this algorithm is doing anymore
			return (int)(5 + Map.random.range(6) + n);
		}
		
		public int trapQuantityPerLevel(int level) {
			if(level > TOTAL_LEVELS) level = TOTAL_LEVELS;
			return 1 + (int)Math.Ceiling((double)level / (double)(2 + Map.random.rangeInt(3)));
		}
		
		/* Returns the zone a level is in */
		public int getLevelZone(int level) {
			level--;
			if(level <= 0) return 0;
			if(level < gameState.levelZones.Count) return gameState.levelZones[level];
			return Map.CHAOS;
		}
...
///				return gameState.itemDungeonContent.seed;
		...
		public static double getLevelXp(int level) {
			if(level > Game.MAX_LEVEL) level = Game.MAX_LEVEL;
